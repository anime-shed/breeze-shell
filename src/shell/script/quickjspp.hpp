...
template <typename R, typename /*_SFINAE*/ = void> struct js_traits {
     ...
     * This function is intentionally not implemented. User should implement
     * this function for their own type.
     * @param v This value is passed as JSValueConst so it should be freed by
     * the caller.
     ...
     * This function is intentionally not implemented. User should implement
     * this function for their own type.
     * @return Returns JSValue which should be freed by the caller or
     * JS_EXCEPTION in case of error.
     */
...
template <typename... Ts> struct js_traits<std::variant<Ts...>> {
    ...
    static std::variant<Ts...> unwrap(JSContext *ctx, JSValueConst v) {
        ...
        case JS_TAG_INT:
            [[fallthrough]];
        case JS_TAG_BIG_INT:
            return unwrapPriority<std::is_integral, std::is_floating_point>(ctx,
                                                                            v);
        case JS_TAG_BOOL:
...
struct js_traits<std::weak_ptr<T>> : js_traits<std::shared_ptr<T>> {
    static JSValue wrap(JSContext *ctx,
                        const std::weak_ptr<T> &value) noexcept {
...
Tuple unwrap_args_impl(JSContext *ctx, int argc, JSValueConst *argv,
    ...
    return Tuple{unwrap_arg_impl<std::tuple_element_t<I, Tuple>, I,
                                 sizeof...(I)>::unwrap(ctx, argc, argv)...};
}
            ...
            [](JSContext *ctx, JSValueConst this_value, int argc,
               JSValueConst *argv) noexcept -> JSValue {
                if constexpr (PassThis)
                    return detail::wrap_this_call<R, Args...>(
...
struct js_traits<ctor_wrapper<T, Args...>> {
    static JSValue wrap(JSContext *ctx, ctor_wrapper<T, Args...> cw) noexcept {
        return JS_NewCFunction2(
            ...
            [](JSContext *ctx, JSValueConst this_value, int argc,
               JSValueConst *argv) noexcept -> JSValue {
                if (js_traits<std::shared_ptr<T>>::QJSClassId ==
                    0) // not registered
                {
#if defined(__cpp_rtti)
                    ...
                    js_traits<std::shared_ptr<T>>::register_class(
                        ctx, typeid(T).name());
#else
                    JS_ThrowTypeError(ctx, "quickjspp ctor_wrapper<T>::wrap: "
                ...
                try {
                    std::shared_ptr<T> ptr = std::apply(
...
template <class T> struct js_traits<std::shared_ptr<T>> {
    ...
    /// Signature of the function to obtain the std::shared_ptr from the
    /// JSValue.
    ...
    /// class' base type.
    inline static std::function<void(JSClassID, ptr_cast_fcn_t)>
        registerWithBase;
    ...
    static void registerDerivedClass(JSClassID derived_class_id,
        ...
        // Instrument the derived class so that it can propagate new derived
        // classes to us.
        auto old_registerWithBase =
        ...
        js_traits<std::shared_ptr<D>>::registerWithBase =
            [old_registerWithBase = std::move(old_registerWithBase)](
                ...
                if (old_registerWithBase)
                    old_registerWithBase(derived_class_id,
                                         derived_ptr_cast_fcn);
    ...
    ensureCanCastToBase(JSContext *ctx) {
        ...
        js_traits<std::shared_ptr<B>>::template registerDerivedClass<T>(
            QJSClassId, unwrap);
    }
     ...
     * These values should be marked by class_registrar::mark for QuickJS
     * garbage collector so that the cycle removal algorithm can find the other
    ...
    static void register_class(JSContext *ctx, const char *name,
        ...
        if (!JS_IsRegisteredClass(rt, QJSClassId)) {
            ...
            if (!markOffsets.empty()) {
                marker = [](JSRuntime *rt, JSValueConst val,
                            JS_MarkFunc *mark_func) {
                    auto pptr = static_cast<std::shared_ptr<T> *>(
                        JS_GetOpaque(val, QJSClassId));
                    assert(pptr);
     ...
     * Creates an object with class if #QJSClassId and sets its opaque pointer
     * to a new copy of #ptr.
     */
    ...
    static std::shared_ptr<T> unwrap(JSContext *ctx, JSValueConst v) {
        ...
        if (obj_class_id == QJSClassId) {
            ...
            JS_ThrowTypeError(ctx,
                              "Expected type %s, got object with classid %d",
...
template <class T> struct js_traits<T *, std::enable_if_t<std::is_class_v<T>>> {
    static JSValue wrap(JSContext *ctx, T *ptr) {
        ...
        if (js_traits<std::shared_ptr<T>>::QJSClassId == 0) // not registered
        {
            ...
            // it probably means you are passing JSValueConst to where JSValue
            // is expected
...
template <> struct js_traits<detail::function> {
    ...
    static void register_class(JSContext *ctx, const char *name) {
        ...
        JSClassDef def{name,
                       // destructor
                       [](JSRuntime *rt, JSValue obj) noexcept {
                           auto fptr = static_cast<detail::function *>(
                               JS_GetOpaque(obj, QJSClassId));
                           assert(fptr);
                           if (fptr->destroyer)
                               fptr->destroyer(fptr);
...
public:
     ...
     * @return type returned by js_traits<std::decay_t<T>>::unwrap that should
     * be implicitly convertible to T
     * */
...
public:
    ...
    Runtime() {
        ...
        JS_SetHostPromiseRejectionTracker(
            rt, promise_unhandled_rejection_tracker, NULL);
...
public:
    ...
    public:
        Module(JSContext *ctx, const char *name) : ctx(ctx), name(name) {
            m = JS_NewCModule(
                ctx, name, [](JSContext *ctx, JSModuleDef *m) noexcept {
                    auto &context = Context::get(ctx);
                    auto it = std::find_if(
                        context.modules.begin(), context.modules.end(),
                    ...
                    auto set_key_recursive =
                        [ctx](this auto &self, Value obj,
                              ...
                              Value &value) -> void {
                        if (keys.size() == 1) {
                            JS_SetPropertyStr(ctx, obj.v, keys[0].data(),
                                              value.v);
                        } else {
                            auto key = keys[0];
                            auto keys_rest = std::vector<std::string_view>(
                                keys.begin() + 1, keys.end());
                            Value sub_obj =
                                JS_GetPropertyStr(ctx, obj.v, key.data());
                            if (JS_IsUndefined(sub_obj.v)) {
                                sub_obj = JS_NewObject(ctx);
                                JS_SetPropertyStr(ctx, obj.v, key.data(),
                                                  sub_obj.v);
                            }
                    ...
                    // resort the exports to ensure that the shorter namespaces
                    // are processed before the longer ones, so that we can
                    // create nested objects correctly
                    std::ranges::sort(
                        it->exports, [](const nvp &a, const nvp &b) {
                            return a.first.size() < b.first.size();
                        });

                    for (auto &e : it->exports) {
                        auto string_path = std::string(e.first);
                        auto path = string_path |
                                    std::views::split(std::string_view("::")) |
                                    std::ranges::to<std::vector<std::string>>();
                        auto view_path = std::vector<std::string_view>(
                            path.begin(), path.end());

                        set_key_recursive(export_obj, view_path, e.second);
                        JS_SetModuleExport(ctx, m, path[0].data(),
                                           JS_GetPropertyStr(ctx, export_obj.v,
                                                             path[0].data()));
                    }
        ...
        Module &add(std::string_view name, JSValue &&value) {
            ...
            JS_AddModuleExport(
                ctx, m, std::string(name.substr(0, name.find("::"))).data());
            return *this;
        }
    ...
    private:
        ...
        public:
            explicit class_registrar(const char *name,
                                     qjs::Context::Module &module,
                                     qjs::Context &context)
            ...
            class_registrar &property(const char *name) {
                js_traits<std::shared_ptr<T>>::template ensureCanCastToBase<
                    FGet>(context.ctx);
                js_traits<std::shared_ptr<T>>::template ensureCanCastToBase<
                    FSet>(context.ctx);
             ...
             * @param name constructor name (if not specified class name will be
             * used)
             */
            template <typename... Args>
            ...
            template <class B> class_registrar &base() {
                static_assert(!std::is_same_v<B, T>,
                              "Type cannot be a base of itself");
            ...
            /** All qjs::Value members of T should be marked by mark<> for
             * QuickJS garbage collector so that the cycle removal algorithm can
             * find the other objects referenced by this object.
             */
...
public:
    ...
    /** returns current exception associated with context and clears it. Should
     * be called when qjs::exception is caught */
    Value getException() {
    ...
    Value evalFile(const char *filename, int flags = 0) {
        ...
        if (!buf)
            throw std::runtime_error{
                std::string{"evalFile: can't read file: "} + filename};
    ...
    Value fromJSON(std::string_view buffer, const char *filename = "<fromJSON>",
                   int flags = 0) {
        assert(buffer.data()[buffer.size()] == '\0' &&
               "fromJSON buffer is not null-terminated"); // JS_ParseJSON
                                                          // requirement
...
struct js_traits<std::function<R(Args...)>, int> {
            ...
            [jsfun_obj = Value{weakFromContext(ctx), JS_DupValue(ctx, fun_obj)},
             weak](Args... args) -> R {
                ...
                auto work = [&]() {
                    ...
                    detail::wrap_args(jsfun_obj.ctx, argv,
                                      std::forward<Args>(args)...);
                    JSValue result =
                        JS_Call(jsfun_obj.ctx, jsfun_obj.v, JS_UNDEFINED, argc,
    ...
    static JSValue wrap(JSContext *ctx, Functor &&functor) noexcept {
        ...
        if (JS_IsException(obj))
            return obj;
        try {
            auto fptr = function::create(JS_GetRuntime(ctx),
            ...
            fptr->invoker = [](function *self, JSContext *ctx,
                ...
                assert(self);
                auto f =
                    reinterpret_cast<std::decay_t<Functor> *>(&self->functor);
                return detail::wrap_call<R, Args...>(ctx, *f, argc, argv);
            };
...
/** Convert from std::map<std::string, T> to Object and vice-versa.
 * Maps string keys to JS object properties with typed values.
 */
template <class T> struct js_traits<std::map<std::string, T>> {
    static JSValue wrap(JSContext *ctx, const std::map<std::string, T> &map) noexcept {
        try {
            auto jsobj = Value{weakFromContext(ctx), JS_NewObject(ctx)};
            for (const auto &[key, value] : map)
                jsobj[key.c_str()] = value;
    ...
    static std::map<std::string, T> unwrap(JSContext *ctx, JSValueConst jsobj) {
        if (!JS_IsObject(jsobj) || JS_IsArray(jsobj)) {
            JS_ThrowTypeError(ctx, "Expected object for map conversion");
            throw exception{ctx};
        }
...
template <typename U, typename V> struct js_traits<std::pair<U, V>> {
    ...
    static std::pair<U, V> unwrap(JSContext *ctx, JSValueConst jsarr) {
        ...
        if (len != 2) {
            JS_ThrowTypeError(ctx,
                              "js_traits<%s>::unwrap expected array of length "
                              "2, got length %d",
...
template <typename... T> struct js_traits<std::tuple<T...>> {
    ...
    static std::tuple<T...> unwrap(JSContext *ctx, JSValueConst jsarr) {
        ...
        int32_t len;
        if (JS_ToInt32(ctx, &len, JS_GetPropertyStr(ctx, jsarr, "length")) <
            0) {
            throw exception{ctx};
        }
        ...
        if (static_cast<size_t>(len) != tuple_size) {
            JS_ThrowRangeError(ctx,
                               "Array length %d does not match tuple size %zu",
                               len, tuple_size);
            throw exception{ctx};
        }
    ...
    static std::tuple<T...> unwrap_impl(JSContext *ctx, JSValueConst jsarr,
        ...
        try {
            ...
            JS_ThrowInternalError(ctx,
                                  "Failed to unwrap tuple element at index %zu",
                                  sizeof...(I) - 1);
            throw;
        }
    }
